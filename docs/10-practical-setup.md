...
```text
{ 模型, 啟用的 MCP 集合, 啟用的代理程式/設定檔 }
```

除非必要，否則不要在昂貴的對話中途更改這些控制項。對話中途切換通常會使已緩存的 prefix 失效，從而失去累積的緩存輸入折扣。

如果必須切換，請按以下步驟操作：

1. 撰寫簡短的手寫摘要（決策、限制條件、下一步行動）。
2. 在新模式中開啟新對話。
3. 僅貼上摘要和所需檔案。
...
專注代理會比一般用途指令集節省更多指令負擔。它們還給你一個穩定的控制面板：相同的任務配置可以宣告它允許使用的工具、它攜帶的指令以及，當你的 Copilot 介面支援時，應該使用的模型。重複程式設計工作流程時，優先使用專注的自訂代理而不是預設代理，當你關心可預測的成本時。預設代理會繼承目前環境的更多內容：活躍工具、提供的介面和目前選定的模型。

將工具清單保持精簡。本儲存體的 `agents/token-saver.agent.md` 即為範例：內建的 `bash`、`edit` 和 `view`；無重複的檔案系統 MCP；簡潔的輸出規則；明確的工具最小化。
...
在 Windows 上，建議團隊使用前應先驗證 RTK。hook 路徑在 PowerShell、Git Bash、WSL 和 VS Code 代理執行之間可能較為脆弱。如果 RTK 增加了設定摩擦或導致命令失敗，請跳過它，並優先關注清晰的設定檔、減少 MCP 伺服器、精確的提示詞和更短的命令輸出。
...
- 審視 VS Code 設定檔與擴充功能 —— 除非目前存儲庫需要，否則停用會注入 AI 技能、代理、MCP 伺服器或工具的擴充功能
- （商業/企業版）為新的敏感路徑審查存儲庫／組織的**內容排除**設定
- 檢查你的模型使用情形 —— 你是否固定了那些 Auto 可以路由至更便宜層級的高耗時模型？
- 在 Copilot CLI 中，注意右下角的 **AIC** 計數器。除以 100 即為約略的美元價值，然後問自己：產出節省的時間是否超過其消耗的成本？若花費高但產出弱，就將其視為對提示詞範圍、上下文大小、工具數量或模型選擇的反饋
...
對於重複的工作流程，請搭配自訂的代理設定檔與乾淨的 VS Code 設定檔使用。停用會注入技能、代理、MCP 伺服器或工具介面的擴充功能，這些都不是編碼所必需的。最可預測的設定是無趣的：一個專注的代理、一個預期的模型，以及僅該儲存庫所需的工具。
