...
如果你必須切換模型路徑或配置（例如從便宜/Auto 切換到進階、從 `medium` 切換到 `high` 推理努力，或變更技能/工具組合），請開啟一個全新的對話，僅附上相關的摘要和檔案。這能保留原始對話中的快取友好穩定性，並避免將冗長且低價值的歷史記錄帶入成本較高的請求中。由於具體的計費實作可能因介面和計劃而異，因此應將快取失效視為需要避免的風險，而非必然的重新計價計算。
