...
| 對話中變更 | 對快取與上下文的影響 |
|--------------------|---------------------------|
| **切換模型** | 最糟的情況。快取通常是按模型計算的，因此快取的前綴實際上已消失 —— 且攜帶的歷史記錄會在新的（通常更昂貴的）通道中重新計價。在工作開始前選擇好模型；請參閱 [模型切換規則](11-models-and-pricing.md#anti-patterns)。 |
| **切換自定義代理** | 交換系統提示詞、工具集和指令 —— 前綴發生變化，因此快取失效，且先前累積的上下文在新的代理下變成污染。 |
| **切換 MCP 伺服器 / 工具** | 更改前綴中的工具定義區塊 —— 快取失效，加上你在之後的每一輪中都會添加或刪除始終加載的 Token。 |
...
- **在對話中途切換模型**（例如，從一個 Claude/GPT 級別移動到另一個）
- **在對話中途啟用或禁用 MCP 伺服器**（工具定義位於上下文頂部附近；更改它們會使大型前綴失效）
- **在對話中途切換代理/設定檔模式**（預設代理 ↔ 自定義代理，或一個自定義代理 ↔ 另一個）

實用規則：在整個長對話中保持此元組固定：

```text
{ 模型, 啟用的 MCP 集合, 啟用的代理/設定檔 }
```
